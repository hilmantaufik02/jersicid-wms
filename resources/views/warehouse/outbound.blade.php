...
@section('title', 'Outbound Dispatch')
@section('page-title', 'Outbound Dispatch')
@section('page-subtitle', 'Scan barcode SKU dan lokasi Bin untuk pengeluaran barang.')

@section('content')
<div x-data="warehouseScanner('{{ route('gudang.outbound.store') }}', 'OUTBOUND')" class="max-w-4xl mx-auto">
    
    <!-- Status Feedback Banner (Sama seperti Inbound) -->
    ...
    <div class="bg-wms-card border border-wms-border rounded-2xl shadow-2xl shadow-cyan-900/10 p-6 md:p-8">
        <form @submit.prevent="submitData" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Batch tidak diperlukan di Outbound -->
            ...
            <button type="submit" 
                ...
                <span x-text="isProcessing ? 'Memproses...' : 'KONFIRMASI OUTBOUND (ENTER)'"></span>
            </button>
        </form>
    </div>
        ...
        <div class="space-y-2">
            <template x-for="log in recentLogs" :key="log.id">
                <div class="flex justify-between items-center bg-wms-bg/50 p-3 rounded-lg border border-wms-border">
                    ...
                    <!-- Warna merah/oranye untuk Outbound -->
                    <span class="text-red-400 font-bold font-mono" x-text="`-${log.qty} PCS`"></span>
                </div>
            </template>
...
<!-- Reusable Alpine.js Component Logic (Sama persis dengan Inbound) -->
<script>
